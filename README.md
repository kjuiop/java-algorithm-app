...
- 풀이 : programmers/Q201404
- 문제 : 충분히 가까운 같은 문자 문제 풀이
- 슬라이딩 윈도우 : 리스트나 문자열과 같이 순서대로 저장된 자료구조에서 일정 길이만큼만 검사하는 알고리즘


문제 풀이

1. 문자열을 배열로 만들기 위해 str.toCharArray() 를 사용한다
2. 맨 앞의 문자를 인덱스만큼 비교하기 때문에 Queue 자료 구조를 선택한다
3. Queue 를 사용할 때에는 que.poll() 사용하여 앞의 요소를 사용하고, while 문에 !que.isEmpty() 를 사용한다
4. k 만큼의 인덱스만큼 비교해야하기 때문에 k 에 대한 조건을 적용한다.